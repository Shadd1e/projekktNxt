...
export async function POST(request) {
  try {
    ...
    const userResult = await pool.query(
      "SELECT id, email, is_paid, plan_type FROM users WHERE id = $1",
      [userId]
    );
    const user  = userResult.rows[0];
    const token = signToken({ userId: user.id, email: user.email, isPaid: user.is_paid, planType: user.plan_type });

    const response = NextResponse.json({
      message: "Email verified successfully.",
      token,
      user: { id: user.id, email: user.email, isPaid: user.is_paid, planType: user.plan_type },
    });
