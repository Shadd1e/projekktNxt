...
export function clearToken() {
  localStorage.removeItem("projekkt_token");
  localStorage.removeItem("projekkt_user");
}
export function saveUser(user) { localStorage.setItem("projekkt_user", JSON.stringify(user)); }
export function getUser() {
  if (typeof window === "undefined") return null;
  try { return JSON.parse(localStorage.getItem("projekkt_user") || "null"); }
  catch { return null; }
}
...
export const api = {
  ...
  initiatePayment: (plan)  => request("/api/payment/initiate",   { method: "POST", body: JSON.stringify({ plan }) }),
};
